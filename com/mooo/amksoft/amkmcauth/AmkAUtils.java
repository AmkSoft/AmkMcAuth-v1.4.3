...
public class AmkAUtils {
    ...
    public static BukkitTask createRegisterReminder(Player p, Plugin pl) {
		...
		if(Config.registrationType.equalsIgnoreCase("email")) {
			RegType = Language.EMAIL_ADDRESS.toString(); //"email address";
		} else {
			RegType = Language.PASSWORD.toString(); //"password";
		}
    ...
    public static BukkitTask createSetEmailReminder(Player p, Plugin pl) {
		String RegType = Language.EMAIL_ADDRESS.toString(); //"email address";
	    return createReminder(p, pl, ChatColor.RED + Language.PLEASE_SETEMAIL_WITH.toString() + ":" + ChatColor.GRAY + " /setemail [" + RegType + "]" + ChatColor.RED + "!", Config.emailRemindInterval * 20L);
    }

    public static BukkitTask createLoginReminder(Player p, Plugin pl) {
		String RegType = Language.PASSWORD.toString(); //"password";
		if(Config.sessionType.contains("HiddenChat"))
	        return createReminder(p, pl, ChatColor.RED + Language.PLEASE_LOG_IN_WITH0.toString() + ":" + ChatColor.GRAY + " t\\login [" + RegType + "]" + ChatColor.RED + "!", Config.remindInterval * 20L);
		else
			return createReminder(p, pl, ChatColor.RED + Language.PLEASE_LOG_IN_WITH1.toString() + ":" + ChatColor.GRAY + " /login [" + RegType + "]" + ChatColor.RED + "!", Config.remindInterval * 20L);
    }

    public static BukkitTask createLoginEmailReminder(Player p, Plugin pl) {
		String RegType = Language.PASSWORD.toString(); //"password";
		if(Config.sessionType.contains("HiddenChat"))
	        return createEmailReminder(p, pl, ChatColor.RED + Language.PLEASE_LOG_IN_WITH0.toString() + ":" + ChatColor.GRAY + " t\\login [" + RegType + "]" + ChatColor.RED + "!", Config.remindInterval * 20L);
		else
			return createEmailReminder(p, pl, ChatColor.RED + Language.PLEASE_LOG_IN_WITH1.toString() + ":" + ChatColor.GRAY + " /login [" + RegType + "]" + ChatColor.RED + "!", Config.remindInterval * 20L);
    }
